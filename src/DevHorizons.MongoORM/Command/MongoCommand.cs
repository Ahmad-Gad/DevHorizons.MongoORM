namespace DevHorizons.MongoORM.Command
{
    ...
    public class MongoCommand<T> : IMongoCommand<T>
    {
        #region Private Fields
        private readonly IMongoConnection mongoConnection;

        private readonly ILogger<MongoCommand<T>> logger;
        ...
        #region Constructors
        public MongoCommand(IMongoConnection mongoConnection)
        {
        ...
        public MongoCommand(IMongoConnection mongoConnection, ILogger<MongoCommand<T>> logger)
        ...
        public bool SetCollection(string collectionName)
        {
            ...
            try
            {
                var coll = this.database.GetCollection<T>(collectionName);
        ...
        public IResult<long> GetTotalCount()
        {
            ...
            try
            {
                var value = this.collection.CountDocuments(Builders<T>.Filter.Empty);
                return new Result<long>
                {
                    Value = value,
                    MatchedCount = value,
        ...
        public async Task<IResult<long>> GetTotalCountAsync()
        {
            ...
            try
            {
                var value = await this.collection.CountDocumentsAsync(Builders<T>.Filter.Empty);
                return new Result<long>
                {
                    Value = value,
                    MatchedCount = value,
        ...
        #region Get Filter
        public IResult<ICollection<T>> Get(string filter, int pageSize = 0, int page = 0)
        {
            ILogDetails error = null;
            if (!this.connected || this.collection is null)
            {
        ...
        public async Task<IResult<ICollection<T>>> GetAsync(string filter, int pageSize = 0, int page = 0)
        {
            ILogDetails error = null;
            if (!this.connected || this.collection is null)
            {
